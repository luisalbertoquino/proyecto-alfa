...
- **Droplet de pruebas:** un Droplet de DigitalOcean (compartido con otros proyectos del usuario) con OpenLiteSpeed + PHP + MariaDB + Redis + PM2 instalados de forma nativa (ver `docs/architecture/infraestructura.md`). Este servidor cumple **hoy, a la vez, el rol de staging y el de "producción" del prototipo** — no hay un tercer ambiente separado (ver `docs/estado-actual.md`, sección "Infraestructura ya disponible"). Se actualiza corriendo `bash scripts/deploy.sh` a mano por SSH (git pull + reinstalar dependencias + migrar + reconstruir + reiniciar PM2) — el pipeline automatizado por GitHub Actions es una extensión planeada, no lo que corre hoy (ver `docs/development/ci-cd.md`).
...
- Cloudflare **no está implementado en el prototipo actual** (era el plan original) — el SSL se emite directo en el droplet con `certbot` (Let's Encrypt) y el DNS apunta directo del proveedor de dominio a la IP del droplet. Se retoma como decisión futura si el tráfico real lo justifica — ver `docs/architecture/infraestructura.md`.
