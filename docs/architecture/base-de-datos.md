...
Cubre: elección de PostgreSQL, estrategia multi-tenant en el modelo de datos, indexación, convención de migraciones seguras, backups y recuperación, y particionamiento/aislamiento futuro de tenants grandes.
...
2. **PostgreSQL como única fuente de verdad transaccional.** Redis es cache y cola, no almacenamiento de verdad (ver `escalabilidad.md`).
...
- PostgreSQL es el motor relacional único del sistema. Se eligió por soporte maduro de JSON (`jsonb`) para atributos variables de producto, particionamiento nativo, y robustez transaccional para operaciones de negocio crítico (pedidos, inventario, pagos).
...
- Se revisan periódicamente índices no usados (`pg_stat_user_indexes`) para no pagar el costo de escritura de índices que ninguna consulta real utiliza.

### Migraciones sin downtime

- Un cambio de esquema que pueda bloquear una tabla grande (agregar columna `NOT NULL` sin default, agregar índice sin `CONCURRENTLY`, cambiar tipo de columna) se descompone en pasos compatibles hacia atrás:
...
- Los índices nuevos sobre tablas con volumen significativo se crean con `CREATE INDEX CONCURRENTLY` para no bloquear escrituras.
...
- Backups automáticos completos con frecuencia diaria como mínimo, más WAL (Write-Ahead Log) continuo para permitir recuperación a un punto en el tiempo (PITR) entre backups completos, dado que un pedido o un pago perdido no es aceptable.
...
- Se evalúa particionar una tabla (por ejemplo, particionamiento nativo de PostgreSQL por rango de fecha en `pedidos` o `movimientos_inventario`) cuando su volumen empiece a degradar el rendimiento de índices o mantenimiento (vacuum, reindex), no de forma preventiva.
...
- Un incidente que corrompe datos de inventario de un tenant se recupera restaurando desde el backup con PITR al minuto anterior al incidente, en un entorno de prueba, antes de decidir si se aplica sobre producción.
