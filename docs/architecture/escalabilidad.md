...
Explicar cómo escala Proyecto Alfa cuando crece el número de tenants y el tráfico: escalado horizontal de instancias de aplicación stateless, cache con Redis, colas para trabajo pesado, CDN de Cloudflare para estáticos, réplicas de lectura de MySQL cuando haga falta, y qué se mide para saber cuándo escalar cada capa.
...
2. **Cachear lo que se lee mucho y cambia poco.** Redis absorbe lectura repetida antes de que llegue a MySQL.
...
- **Estado actual del prototipo:** hoy Proyecto Alfa corre en un solo Droplet con despliegue nativo (OpenLiteSpeed + PHP + MySQL + Redis, sin contenedores — ver `docs/architecture/infraestructura.md` y `docs/adr/ADR-002.md`), suficiente para el tráfico del piloto en Fase 2. Este documento fija la estrategia de escalado horizontal como la palanca **futura** para cuando el negocio la justifique (más tráfico o tenants del que un solo servidor puede sostener), no como algo que el prototipo necesite hoy.
- Las instancias de `apps/api`, `apps/web` (SSR) y `apps/admin` se diseñan stateless: cualquier instancia puede atender cualquier request sin depender de sesión ni cache local en disco, precisamente para que el día que haga falta, agregar servidores (nativos o, si se adopta esa estrategia, contenedores) sea una decisión de infraestructura y no una reescritura de código.
- El escalado horizontal (agregar o quitar instancias/servidores) es la respuesta por defecto ante más tráfico concurrente cuando llegue ese momento, antes de considerar aumentar recursos de un solo servidor (escalado vertical).
...
- Todo el contenido estático y las páginas SSG/ISR de `apps/web` se sirven detrás de Cloudflare, de forma que el tráfico de catálogo de alto volumen (ej. una campaña de publicidad exitosa) llegue a la CDN y no al servidor de aplicación en la mayoría de los casos.
...
### Réplicas de lectura de MySQL

- El punto de partida es una sola instancia de MySQL (primaria) para todas las lecturas y escrituras — no se introduce complejidad de réplicas antes de necesitarla.
...
- Las consultas que van a réplica de lectura son las que toleran una pequeña latencia de replicación (reportes, listados no críticos); las operaciones que requieren consistencia inmediata (confirmar un pedido, verificar stock antes de vender) siempre leen de la primaria. La replicación de MySQL (basada en binlog) resuelve el mismo problema que la streaming replication de PostgreSQL, con el mismo principio de qué consultas puede tolerar la latencia y cuáles no.
...
- **Campaña publicitaria exitosa un fin de semana:** el tráfico a la ficha de producto lo absorbe Cloudflare porque la página es SSG/ISR; el tráfico que sí llega a `apps/api` (agregar al carrito, checkout) se atiende, cuando el tráfico del negocio lo justifique, agregando instancias de `apps/api` de forma horizontal, sin cambios de código — hoy, en Fase 2, el único servidor del droplet es suficiente para el tráfico del piloto.
...
- Herramienta de autoescalado horizontal (y si se adopta orquestación por contenedores como mecanismo para lograrlo) una vez exista tráfico real que lo justifique; hoy, con un solo servidor nativo, el escalado no aplica y cualquier ajuste de capacidad es manual/operativo.
...
- `docs/architecture/infraestructura.md` — dónde y cómo corre hoy el servidor único del prototipo, y qué cambiaría al escalar a más de uno.
