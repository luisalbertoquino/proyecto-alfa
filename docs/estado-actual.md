...
**Falta que el usuario pruebe en su navegador real** el flujo interactivo de las Semanas 2 y 3 (agregar al carrito y completar checkout en `apps/web`; iniciar sesión, crear/editar productos y confirmar/cancelar pedidos en `apps/admin`) — todo lo que se pudo probar sin navegador (endpoints con `curl`, aislamiento entre tenants, HTML generado en servidor) ya está verificado. Una vez confirmado que se ve y funciona bien a ojo, seguir con la Semana 4: cargar el catálogo real (no el de ejemplo) y hacer una pasada de pruebas end-to-end completa.
